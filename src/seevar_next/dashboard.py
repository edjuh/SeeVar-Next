...
def render_dashboard(config_path: Path = Path("config/seevar-next.json"), data_dir: Path = Path("data")) -> str:
    ...
    steps_text = _read_text(data_dir / "flight_steps.txt")
...
<section>
<h2>Workflow Dry Run</h2>
<pre>{html.escape(steps_text)}</pre>
</section>
<section>
<h2>Flight</h2>
...
class DashboardHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        files = {
            ...
            "/flight_steps.txt": (self.data_dir / "flight_steps.txt", "text/plain"),
            "/flight_steps.json": (self.data_dir / "flight_steps.json", "application/json"),
